...
- The stale case uses the ordinary timestamp B-tree for both phases and temporarily grows the table to 10.5 million rows, then deletes the extra 500,000 and vacuums/analyzes. This is the specification's append experiment; the base matrix stays at 10 million. Six runs per phase allow median reporting. Plans are never forced, and join changes or speedups are not guaranteed.
